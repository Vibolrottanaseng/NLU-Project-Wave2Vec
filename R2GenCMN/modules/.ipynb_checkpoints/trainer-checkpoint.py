...
class BaseTrainer(object):
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        if save_best or True:
            import json
            save_path = os.path.join(self.checkpoint_dir, 'test_results_best.json')

            try:
                with open(save_path, 'w') as f:
                    json.dump(self.latest_test_results, f, indent=4)
                self.logger.info(f"Saved best test results to {save_path}")
    ...
    def _train_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.test_dataloader):
                ...
                output, _ = self.model(images, mode='sample')
                generated_reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
                test_res.extend(generated_reports)
                test_gts.extend(ground_truths)

                for i in range(len(generated_reports)):
                    results.append({
                        "id": str(images_id[i]),   # <-- your real ID (important!)
